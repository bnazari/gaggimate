...
Method: new branch `silvia` at upstream/master (670d05a3, 11 commits past the old
fork point), final tree re-staged thematically — no rebase, no conflict re-resolution.
Every commit builds standalone (controller + display verified per commit; sim on its
commit).

**Commits:** (1) Rancilio UI theme (EEZ regen + web logos/themes), (2) mode-status
LEDs incl. the coalescing-key split, LedControlPlugin capability gate + cache resync,
and reconnect LED resync, (3) momentary buttons / water rocker / 2s hold flush /
pump-off-on-water-exit, (4) simulator MachinePanel + host fixes, (5) upstream-candidate
fixes (BLE bounce, semver abort, stale WebUI blob, frozen imgbtn), (6) docs.

**Deliberately dropped from `silvia`** (archived on `hw_scale`, restore when the scale
hardware installs): the dragm83 hardware-scales merge (protocol v4, HX711 peripheral,
web calibration UI), the XIAO ESP32-C3 controller-mock, dragm83's release/flasher
infra, and the TFT_eSPI dep (upstream removed it). Base gains upstream's 11 new
commits (BLE transport fixes, MQTT loop fix, profile transition fixes, MachineTab
color styles) and, by taking upstream's DefaultUI, restores the version-string
`stringChanged` guards the old merge had lost.

**New in this rebuild (LED commit):** the mode-LED ↔ Sunrise queue collision found in
the 8/19 LED audit is fixed via a per-group LED coalescing key (Protocol.h), the
Sunrise LedControlPlugin no longer transmits on boards without the ledControl
capability, and its change-cache resyncs after reconnect.

**Verification:** per-commit builds + final sim screenshot run + empty-delta check
against `hw_scale` for all ported hunks (see below). `hw_scale` left untouched on
origin as the true pre-rebuild history.

---

## 2026-08-19 (night) — Display crash analyzed from coredump: BLE-scale use-after-free on brew end

**Report:** machine installed and working; display reset once — user long-pressed
brew (flush) from standby, display rebooted at some point after.

**Analysis (no changes made, per user request):** pulled the coredump over WiFi via
`GET /api/core-dump` (37,604 B) — the raw partition image needs wrapping in the
support-bundle JSON (`{"coredump": base64}`) before `scripts/analyze_coredump.py`
accepts it. Backtrace against the flashed ELF (icon-fix build):
`Controller::loopLogic → dispatchEvents (controller:brew:end) → BLEScalePlugin
lambda → BookooScales::stopTimer → sendMessage → NimBLERemoteCharacteristic::
writeValue(this=0xd44658f2 /* garbage */)` — use-after-free. When the flush ENDED,
the plugin told the Bookoo scale to stop its timer; the scale's BLE connection died
concurrently and NimBLE (its own task) had freed the remote objects. The
`scale->isConnected()` guard at BLEScalePlugin.cpp:98 is check-then-use across
tasks — race window of microseconds, hence rare. Upstream bug (upstream's own
`delay(50)` band-aid in `BLEScalePlugin::disconnect()` hints at this race family).
Standby/long-press was incidental — any shot/flush end with a concurrently-dropping
scale can crash.

**Impact:** display reboot only; controller (heater/pump safety) unaffected.

**Pending:** fix candidate (invalidate scale object from the disconnect callback
before NimBLE frees remotes, or serialize scale access) — upstream-relevant; user
...
1. Point `RELEASE_URL` at `https://github.com/bnazari/gaggimate/releases/`.
2. Reconcile one asset name: `hardware-scales.yml` publishes `controller-firmware.bin`
   but `GitHubOTA` downloads `board-firmware.bin`.
3. Publish releases by pushing a `v*-hwscales.N` tag (the workflow from the dragm83
   merge already builds and uploads all assets on tag push).
4. Before trusting it: sanity-check GitHubOTA's semver "is newer" comparison against
   `-hwscales.N` version strings (the July parser guard makes them non-crashing, but
   ordering logic is untested).
